...
def generate_hive_ddl(table, columns,location_url ,output_dir="ddl_scripts"):
    ...
    ddl_file = os.path.join(output_dir, f"ods_{table}_full.sql")

    # 生成Hive DDL语句
    hive_ddl = f"-- 自动生成的Hive DDL脚本\n"
    hive_ddl += f"use gmall;\n"
    hive_ddl += f"-- 源表: gmall.{table}\n"
    hive_ddl += f"-- 目标表: ods_{table}_full\n"
    hive_ddl += f"-- 生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    hive_ddl += f"CREATE TABLE IF NOT EXISTS ods_{table}_full (\n"
...
def generate_seatunnel_config(mysql_host, mysql_port, mysql_user, mysql_password,
    ...
    # 表名映射关系
    table_mapping = f'        "{table}" = "ods_{table}_full",'

    config_content = f"""# 自动生成的SeaTunnel配置文件
# 源表: {database}.{table}
# 目标表: ods_{table}_full
...
sink {{
  Hive {{
    table_name = "{database}.ods_{table}_full"
...
if __name__ == "__main__":
    ...
    # 使用固定值
    mysql_host = "192.168.70.142"
    mysql_port = "3306"
    hdfs_port = "8020"
    mysql_password = "root"
    hive_metastore = "thrift://192.168.70.142:9083"
    hdfs_nn = "hdfs://192.168.70.142:8020"
    database = "gmall"
    ...
    for table in tables:
        ...
        location_url = f"LOCATION '{hdfs_nn}/{HDFS_FILE_PATH}/ods_{table}_full'\n"
        # 生成Hive DDL文件（单独输出）
        generate_hive_ddl(
            table=table,
            columns=columns,
