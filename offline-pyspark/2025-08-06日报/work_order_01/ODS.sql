...
-- 分批次生成数据，每次2万条，共5批次（总10万条）
INSERT INTO TABLE ods_commodity_visit_behavior
SELECT row_number() OVER () + ${batch_num} * 20000 AS id,                -- 批次偏移量，避免ID重复
       ...
       CASE WHEN rand() > 0.3 THEN 2 ELSE 1 END    AS terminal_type,     -- 终端类型区分PC/无线端
       cast(CASE
                WHEN rand() > 0.8 THEN rand() * 240 + 60
                WHEN rand() > 0.2 THEN rand() * 50 + 10
                ELSE rand() * 7 + 3
           END AS INT)                             AS stay_duration,     -- 符合平均停留时长统计逻辑
       CASE WHEN rand() > 0.2 THEN 1 ELSE 0 END    AS is_click,          -- 用于计算跳出率
       CASE WHEN rand() > 0.6 THEN 1 ELSE 0 END    AS is_micro_detail,   -- 微详情访问标识
       CASE
           WHEN rand() > 0.6 THEN cast(rand() * 57 + 3 AS INT)
           ELSE 0
           END                                     AS micro_detail_stay, -- 微详情停留时长（≥3秒）
...
FROM (
         ...
         CROSS JOIN (SELECT 1 AS num
                     ...
                     SELECT 1) t3
         CROSS JOIN (
    -- 增加20条记录，使总条数为10×10×10×20=20000
    SELECT 1 AS num
    UNION ALL
    SELECT 1
    UNION ALL
