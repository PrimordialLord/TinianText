...
-- 向ADS层评价体验表插入数据（基于现有DWS表推导）
INSERT OVERWRITE TABLE ads_evaluation_experience PARTITION (stat_date = '2025-01-31')
SELECT
    all_products.product_id,
    -- 评分分布：基于销量和点击量推导（假设高销量对应高评分）
    COALESCE(
            CONCAT(
                    '{"1星":', ROUND(star1_ratio, 4),
                    ',"2星":', ROUND(star2_ratio, 4),
                    ',"3星":', ROUND(star3_ratio, 4),
                    ',"4星":', ROUND(star4_ratio, 4),
                    ',"5星":', ROUND(star5_ratio, 4), '}'
            ),
            '{"1星":0.0,"2星":0.0,"3星":0.0,"4星":0.0,"5星":0.0}'
    ) AS score_distribution,
    -- 正面评价率：基于销量趋势推导
    COALESCE(ROUND(positive_rate, 4), 0.0) AS positive_rate,
    -- 主要投诉点：基于SKU销量占比异常推导（如低销量SKU可能对应投诉）
    COALESCE(main_complaint, '无投诉') AS key_complaint,
    -- 评价趋势：基于近7天销量趋势推导（使用Hive数组索引语法）
    COALESCE(evaluation_trend, '无数据') AS evaluation_trend
FROM (
         -- 获取所有商品ID（从商品核心汇总表）
         SELECT DISTINCT product_id FROM dws_product_core_summary WHERE stat_date = '2025-01-31'
     ) all_products
         LEFT JOIN (
    -- 先计算每个商品的最小SKU销量占比（独立子查询处理窗口函数）
    SELECT
        p.product_id,
        p.daily_sales,
        p.7d_sales_trend,
        -- 关联SKU的最小销量占比（提前计算窗口函数结果）
        sku_min.sku_min_ratio,
        -- 推导评分分布
        CASE WHEN p.daily_sales = 0 THEN 0.0 ELSE 0.05 END AS star1_ratio,
        CASE WHEN p.daily_sales = 0 THEN 0.0 ELSE 0.05 END AS star2_ratio,
        CASE WHEN p.daily_sales = 0 THEN 0.0 ELSE 0.1 END AS star3_ratio,
        CASE WHEN p.daily_sales = 0 THEN 0.0 ELSE 0.3 END AS star4_ratio,
        CASE WHEN p.daily_sales = 0 THEN 0.0 ELSE 0.5 END AS star5_ratio,
        -- 正面评价率
        CASE WHEN p.daily_sales = 0 THEN 0.0 ELSE 0.8 END AS positive_rate,
        -- 主要投诉点（基于提前计算的最小占比）
        CASE
            WHEN sku_min.sku_min_ratio < 0.01 THEN '质量问题'
            ELSE '无明显投诉'
            END AS main_complaint,
        -- 评价趋势
        CASE
            WHEN (p.7d_sales_trend[6] - p.7d_sales_trend[0]) / NULLIF(p.7d_sales_trend[0], 0) > 0.1
                THEN '明显上升'
            WHEN (p.7d_sales_trend[6] - p.7d_sales_trend[0]) / NULLIF(p.7d_sales_trend[0], 0) > 0
                THEN '略有上升'
            WHEN (p.7d_sales_trend[6] - p.7d_sales_trend[0]) / NULLIF(p.7d_sales_trend[0], 0) < -0.1
                THEN '明显下降'
            WHEN (p.7d_sales_trend[6] - p.7d_sales_trend[0]) / NULLIF(p.7d_sales_trend[0], 0) < 0
    ...
    FROM dws_product_core_summary p
             -- 子查询：提前计算每个商品的最小SKU销量占比（解决窗口函数与GROUP BY冲突）
             LEFT JOIN (
        SELECT
            product_id,
            MIN(sales_ratio) AS sku_min_ratio  -- 每个商品的最小SKU销量占比
        FROM dws_sku_sales_summary
        WHERE stat_date = '2025-01-31'
        GROUP BY product_id
    ) sku_min ON p.product_id = sku_min.product_id
    WHERE p.stat_date = '2025-01-31'
      AND size(p.7d_sales_trend) = 7  -- 确保数组长度为7
    GROUP BY p.product_id, p.daily_sales, p.7d_sales_trend, sku_min.sku_min_ratio
) eval_data ON all_products.product_id = eval_data.product_id;

select * from  ads_evaluation_experience;